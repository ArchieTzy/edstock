$(function () {
    ...
    $('#select').on('click',function () {
        ...
        $("input[type=checkbox]:checked").each(function(i){
            ...
            if($.isNumeric(id)) {
                $.ajax({
                    url: base_url + '/admin/Requests/getItem/' + id,
                    type: "GET",
                    dataType: 'json'
                })
    ...
    $('#requests-form').on('submit',function (e) {
        ...
        if(id != undefined){
            url = base_url + '/admin/Requests/edit/'+ id;
        }else{
            url = base_url + '/admin/Requests/add';
        }
        $.ajax({
            processData: false,
            contentType:false,
            ...
            .done(function(data, textStatus, jqXHR){
                if(data.result=='success'){
                    ...
                    setTimeout(function() {
                        window.location = base_url + '/admin/requests';
                    }, 2000);
                }else{
    ...
    $('#upload-form').on('submit',function (e) {
        ...
        var fd = new FormData(this);
        var id = $('#id').val();
        var url = base_url + '/admin/Requests/uploadFile/'+ id;
    ...
    $('#requests-table').on('click', '.delete', function (e) {
        ...
        isDelete(function (confirmed) {
            if (confirmed) {
                $.ajax({
                    url: base_url + '/admin/Requests/delete/'+ id,
                    type: "DELETE",
                    dataType: 'json',
                    headers : {
    ...
    $('#delete-file').on('click',function (e) {
        ...
        isDelete(function (confirmed) {
            if (confirmed) {
                $.ajax({
                    url: base_url + '/admin/Requests/deleteFile/'+ id,
                    type: "GET",
                    dataType: 'json'
                })
                    .done(function(data, textStatus, jqXHR){
    ...
    $('#requests-table').on('click', '.print', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
        window.location = base_url + '/admin/Requests/printRequest/'+ id;
    });
    ...
    $('.edit-minus').on('click',function(e){
        ...
        if (!isNaN(currentVal)) {
            ...
            if(parseInt(input.val()) == 0) {
                ...
                $.ajax({
                    url: base_url + '/admin//Requestdetails/delete/'+ id.val(),
                    type: "DELETE",
                    dataType: 'json',
                    headers : {
                        'X-CSRF-Token': $('[name="_csrfToken"]').val()
...
function getRequests() {
    $("#requests-table").dataTable({
        ...
        "order": [[ 0, "asc" ]],
        "ajax": {
            "url": base_url + '/admin/Requests/getRequests'
        },
        "columns": [
...
function getItems() {
    $("#items-table").dataTable({
        ...
        "order": [],
        "ajax": {
            "url": base_url + '/admin/Requests/getItems'
        },
        "columns": [
            { data: null,render: function(data,type,row){
